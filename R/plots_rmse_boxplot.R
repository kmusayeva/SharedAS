...
rmse_boxplot <- function(obj, plot_name, d, nexps, start, end, width, height, is_mch = FALSE) {
  ...
  gp <- gp + theme_minimal() + theme(
    panel.grid.major = element_line(size = 0.01), panel.grid.minor = element_line(size = 0.01), legend.position = c(0.85, 0.6), legend.key.size = unit(0.5, "cm"),
