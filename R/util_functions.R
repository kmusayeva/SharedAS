...
wrapper <- function(x, fn, which_distr, norm_params=NULL, params=NULL) {
  ...
  if(which_distr == "norm") x <- sigmoid(x) #map to [0,1]
  else {x <- (x+1)/2; x <- pmin(pmax(x, 0), 1)}

  if(is.null(params)) {res <- t(apply(x, 1, function(x) do.call(fn, list(x)))) }
  else {res <- t(apply(x, 1, function(x) do.call(fn, list(x, params)))) }
...
handle_data <- function(fn, X = NULL, input_dim, num_obj, distr, params=NULL){
  ...
  Y_mean <- colMeans(Y); Y_std <- apply(Y, 2, sd); norm_params <- list(mean=Y_mean, std=Y_std)
...
handle_grads <- function(fn, X, which_distr, is_norm=FALSE, norm_params=NULL, params=NULL){
  ...
  if(!is.null(params$t)) {grads <- grads[,-c(1:ncol(X))]} # for the penicillin problem
...
min_max_norm <- function(A) {
  ...
  if((max(A)-min(A))!=0) return((A-min(A)) / (max(A)-min(A)))
...
rmse_wrapper <- function(Y, Yest, params) {
  ...
  }  else rmses <- rmse(as.matrix(Y), as.matrix(Yest))
