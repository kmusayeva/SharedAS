...
summary_plot <- function(fn, X, Y, which_distr, evectors, methods, width, height, Yout) {
  ...
  for(im in 1:length(evectors)) {
      ...
      legend(-9.5, 53, legend=c("Original", "1d AS", "2d AS"), col=c(rgb(0, 0, 0.5, 0.1),"red","green"), pch=rep(19,3), cex=1.4,
...
coords_poly <- function() {
  ...
  ids0 <- c(77381,  7837,  4458, 95484, 42569, 83219, 38600, 75734, 53170, 62702, 57568,  7620, 76192, 98391, 58887, 90857, 52245,  649, 59481, 42699, 36679, 40649, 39660,
